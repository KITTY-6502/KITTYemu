...
_UpdatePage
    ...
    __loop
        ...
        __loopend
        lda <PageDIST+1>; cmp 32; beq (newStart)
    bra (loop)
    __break
rts
__newStart
...
_DrawScreen
    ...
    __PreCursor
        ...
        ___noinc
        inc <R0+0>; bne (noinc2)
            inc <R0+1>; lda <R0+1>; cmp $6C; bne (notend)
            jmp [End]
            ____notend
        ___noinc2
    bra (PreCursor)
    ...
    __PostCursor
        ...
        inc <R0+0>; bne (noinc2)
            inc <R0+1>
            lda <R0+1>; cmp $6C; beq (End)
        ___noinc2
    bra (PostCursor)
    __GoEND
        lda <R0+1>; cmp $6C; beq (break)
        lda ' '; sta [<R0>+Y]
        inc <R0+0>; bne (noinc)
            inc <R0+1>
        ___noinc
...
_IRQ
    ...
    lda [kShift+X]; sta <char_cur>
    bra (modend)
    ___noshift
    lda $80; bit <keyboard_cache+3>; beq (modend)
    lda <char_cur>; sta <char_repeat>; lda 25; sta <char_repeat_timer>
